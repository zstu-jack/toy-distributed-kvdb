...
// rf.Start(command interface{}) (CommandIndex, Term, isleader)
...
//	return "CommandIndex=" + strconv.Itoa(result.CommandIndex) + ", Term=" + strconv.Itoa(result.Term) + ", IsLeader=" + strconv.Itoa(result.IsLeader)
//}
//
...
//	me        int                 // this peer's CommandIndex into peers[]
//
...
//	log         []LogEntry // first Log' CommandIndex = 1.
//
//	// volatile state for all servers
//	commitIndex int // init = 0;  highest CommandIndex to be committed
//	lastApplied int // init = 0;  highest CommandIndex have been committed.
//
//	// volatile state for leader
//	nextIndex  []int // init = last Log's CommandIndex + 1	    CommandIndex of next Log entry to send to that server
...
//	Term              int
//	LeaderId          int
//	PrevLogIndex      int // Log CommandIndex preceding new ones
//	PrevLogTerm       int
//	Entries           []LogEntry
//	LeaderCommitIndex int // CommandIndex of highest Log entry known to be committed
//}
//
//func (args *AppendEntriesArgs) ToString() string {
//	firstEntryIndex := -1
//	if len(args.Entries) > 0 {
//		firstEntryIndex = args.Entries[0].CommandIndex
//	}
...
//		", entries' size=" + strconv.Itoa(len(args.Entries)) + ", first entry CommandIndex=" + strconv.Itoa(firstEntryIndex)
//}
//
...
//// server is the CommandIndex of the target server in rf.peers[].
...
////
//// the first return value is the CommandIndex that the command will appear at
...
//		isLeader = true
//	}
//	return commandResult.CommandIndex, commandResult.Term, isLeader
//}
//
////
...
//	defer rf.mu.Unlock()
//
//	offset := rf.nextIndex[To] - rf.log[0].CommandIndex
...
//	args := &AppendEntriesArgs{Term: rf.currentTerm, LeaderId: rf.me, PrevLogIndex: rf.log[offset-1].CommandIndex,
...
//			LastLogTerm: rf.log[len(rf.log)-1].Term, LastLogIndex: rf.log[len(rf.log)-1].CommandIndex}
...
//	if len(rf.log) == 0 {
//		rf.log = append(rf.log, LogEntry{Term: 0, CommandIndex: 0})
//	}
//
//	// volatile state for leader
...
//		rf.nextIndex[i] = rf.log[len(rf.log)-1].CommandIndex
//		rf.matchIndex[i] = 0
//	}
//
...
//
//				reply.Who = rf.me
//				reply.NextIndex = rf.log[len(rf.log)-1].CommandIndex + 1
//				reply.Success = 0
...
//				if request.PrevLogIndex > rf.log[len(rf.log)-1].CommandIndex {
//					requestAppendEntries.DoneChan <- 1
//					break
//				}
//
//				offset := prevLogIndex - rf.log[0].CommandIndex
...
//							reply.NextIndex = rf.log[i].CommandIndex + 1
//							break
//						}
//					}
//					requestAppendEntries.DoneChan <- 1
...
//					last := rf.log[len(rf.log)-1].CommandIndex
...
//
//					// TODO: replicate code.
//					offsetApplied := rf.lastApplied - rf.log[0].CommandIndex
//					offsetCommitted := rf.commitIndex - rf.log[0].CommandIndex
//					for i := offsetApplied + 1; i <= offsetCommitted; i++ {
//						DPrintf("================================== (%v) Log Apply : index=%v term=%v len(log)=%v, Log=%v", rf.me, rf.log[i].CommandIndex, rf.log[i].Term, len(rf.log), rf.log[i].Log)
//						msg := ApplyMsg{CommandValid: true, Command: rf.log[i].Log, CommandIndex: rf.log[i].CommandIndex}
//						rf.applyChan <- msg
//						rf.lastApplied = rf.log[i].CommandIndex
//					}
//				}
//
//				requestAppendEntries.DoneChan <- 1
...
//					newer = req.LastLogIndex >= rf.log[len(rf.log)-1].CommandIndex
//				} else {
...
//								continue
//							}
//							if rf.matchIndex[j] >= rf.log[i].CommandIndex {
//								nReplicated++
//							}
//						}
...
//				}
//				for i := offsetApplied + 1; i <= offsetCommitted; i++ {
//					rf.commitIndex = rf.log[i].CommandIndex
//					DPrintf(" ==================================(%v) Log Apply index=%v term=%v, len(log)=%v, Log=%v", rf.me, rf.log[i].CommandIndex, rf.log[i].Term, len(rf.log), rf.log[i].Log)
//					applyMsg := ApplyMsg{CommandValid: true, Command: rf.log[i].Log, CommandIndex: rf.log[i].CommandIndex}
...
//						rf.nextIndex[i] = rf.log[len(rf.log)-1].CommandIndex + 1 // BUG: initialize the nextIndex/matchIndex once the node become leader.
//						rf.matchIndex[i] = 0
//					}
//
//					for i := 0; i < len(rf.peers); i++ {
...
//				if rf.raftState != Leader {
//					result := ClientCommandResult{CommandIndex: 0, Term: rf.currentTerm, IsLeader: 0}
//					clientCommand.ClientCommandResultChan <- result
//				} else {
//					lastLogIndex := rf.log[len(rf.log)-1].CommandIndex
//					rf.log = append(rf.log, LogEntry{Log: clientCommand.Command, Term: rf.currentTerm, CommandIndex: lastLogIndex + 1})
//					rf.nextIndex[rf.me] = rf.log[len(rf.log)-1].CommandIndex + 1
//					rf.matchIndex[rf.me] = rf.log[len(rf.log)-1].CommandIndex
//					rf.persist()
//
//					result := ClientCommandResult{CommandIndex: lastLogIndex + 1, Term: rf.currentTerm, IsLeader: 1}
//					clientCommand.ClientCommandResultChan <- result
